...
async function load(): Promise<Testimonial[]> { try { return JSON.parse(await fs.readFile(dataPath,'utf8')); } catch { return []; } }
async function save(list: Testimonial[]) { await fs.mkdir(path.dirname(dataPath), { recursive: true }); await fs.writeFile(dataPath, JSON.stringify(list, null, 2)); }
