...
async function load(): Promise<Service[]> { try { const buf = await fs.readFile(dataPath,'utf8'); const json = JSON.parse(buf); return Array.isArray(json)? json: []; } catch { return []; } }
async function save(data: Service[]) { await fs.mkdir(path.dirname(dataPath), { recursive: true }); await fs.writeFile(dataPath, JSON.stringify(data, null, 2)); }
